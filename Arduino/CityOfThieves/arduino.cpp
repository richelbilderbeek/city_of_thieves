...
void Arduino::ConnectTo(const Chapter& chapter)
{
  chapter.SetObserver(this);
}

void Arduino::ConnectTo(const Game& game)
{
  game.SetObserver(this);
}

void Arduino::CharacterChanged(const Character& /* character */)
{
  #ifndef NDEBUG
...
int Arduino::GetNumberOfCharsPerLine() const noexcept
{
#ifndef ARM9
  return 80;
#else
...
Option Arduino::RequestOption(const std::vector<Option>& options)
{
...
int Arduino::SlotRequestInput(const std::vector<int>& valid_inputs)
{
...
void Arduino::ShowText(const std::string& text)
{
  if (m_verbose) { std::clog << "Arduino::ShowText: showing " << text << std::endl; }
  if (m_silent) return;
...
void Arduino::SpeakText(const std::string& text)
{
